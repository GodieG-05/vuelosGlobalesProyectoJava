...
public class AirportsConsoleAdapter {
    ...
    public String correctPrimary(String txt, String errMessage, Scanner sc, String tableName){
    ...
    public String existsId(String txt, String errMessage, Scanner sc, String tableName){
        List<String> IDsLsit = airportService.getIDs(tableName);
    ...
    public void registrarAeropuerto(String header, Scanner sc, String rta){
        while (!rta.isEmpty()) {
            Main.clearScreen();
            System.out.println(header);
            String id = correctPrimary("\nIngrese el id del aeropuerto: ", 
            "Este id ya fue utilizado, intentelo de nuevo", sc, "airports");
            System.out.print("Ingrese el nombre del aeropuerto: ");
            String name = sc.nextLine();
            System.out.println("\nCiudades:");
            String idCity = existsId("\nIngrese el id de la ciudad: ", 
            "Ciudad no encontrada, Intente de nuevo", sc, "cities");
    ...
    public void actualizarAeropuerto(String header, Scanner sc, String rta){
        while (!rta.isEmpty()) {
            Main.clearScreen();
            System.out.println(header);
            System.out.println("Aeropuertos:\n");
            String id = existsId("\nIngrese el id del aeropuerto a actualizar: ", 
            "Aeropuerto no encontrado, Intente de nuevo",sc, "airports");
    ...
    public void consultarAeropuerto(String header, Scanner sc, String rta){
        while (!rta.isEmpty()) {
            Main.clearScreen();
            System.out.println(header);
            System.out.println("Aeropuertos:\n");
            String id = existsId("Ingrese el ID del aeropuerto a consultar: ", 
            "Aeropuerto no encontrado, Intente de nuevo", sc, "airports"); 
            Optional<Airports> selectedAirport = airportService.getAirportsById(id);
    ...
    public void elimninarAeropuerto(String header, Scanner sc, String rta){
        while (!rta.isEmpty()) {
            Main.clearScreen();
            System.out.println(header);
            System.out.println("Aeropuertos:\n");
    ...
    public void start() {
        ...
        while (isActive) {
            ...
            switch (op) {
                case 1:
                    registrarAeropuerto(header, sc, rta);
                    break;
                case 2:
                    actualizarAeropuerto(header, sc, rta);
                    break;
                case 3:
                    consultarAeropuerto(header, sc, rta);
                    break;                   
                case 4:
                    elimninarAeropuerto(header, sc, rta);
                    break;      
                case 5:    
                    isActive = false;     
                    break; 
